...
## ✅ Фаза 5: Редактирование задач - ВЫПОЛНЕНО
- Реализована страница-карточка задачи с полным функционалом редактирования
- Форма редактирования YAML метаданных задачи:
  ...
  - эпик (не редактируемое)
  - теги (редактируемое через MultiSelect)
- Markdown редактор с режимами просмотра и редактирования
...
### ✅ Фаза 8: Работа с фазами задач — ВЫПОЛНЕНО
- Backend API для получения отчетов фаз: GET /api/tasks/:id/phases/:phaseId
- Компонент PhaseReportModal для отображения полного содержимого отчетов фаз
- Инлайн кнопки "Посмотреть отчет" в списке фаз на странице задачи
- Компонент EditableMarkdown с инлайн кнопками редактирования у заголовков фаз
- Компонент PhaseEditModal для редактирования текста фаз с Monaco Editor
- Интеграция с существующим API PUT /api/tasks/:id для сохранения изменений
