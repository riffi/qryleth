...
Выполнено:
- Добавлено руководство по координатам: `docs/features/scene-management/terrain-coordinates.md` (оси, центр мира, диапазоны X/Z; примеры размещения; «Частые ошибки»; рекомендации по миграции).
- Создано миграционное руководство: `docs/migrations/024-terrain-coordinates-refactor.md` (height→depth, area.height→area.depth, worldDepth в opts, примеры до/после).
- Обновлён docs/README.md: ссылки на руководство по координатам и миграцию.
- Актуализированы комментарии в коде ранее (фазы 1–2): предупреждения и примеры по depth/центрированию координат.
...
Выполнено:
- Проверены и обновлены примеры в документации: `terrain-in-scripting-panel.md`, `scripting-panel.md`, `scene-api.md`, `api/types/terrain.md` — примеры теперь с `world.depth`, корректными центрами и `area.depth`; исправлены некорректные поля (`uniform.center` → `ring.center`).
- Добавлены новые документы: `features/scene-management/terrain-coordinates.md` (руководство по координатам) и `migrations/024-terrain-coordinates-refactor.md` (миграция).
- Оглавление docs (`docs/README.md`) дополнено ссылками на новое руководство и перечень миграций.
- Актуальность ссылок проверена; примеры центрированы относительно мира.
