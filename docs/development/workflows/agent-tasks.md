...
| **Содержимое**    | Цель, high-level-вехи, перечень задач со статусами и ссылками(если задача поставлена), ссылка на дизайн-док (если есть) + статус выполнения. |
...
- Обязательный раздел «Список фаз» — ПЛАН работ. Для каждой фазы при создании задачи нужно описать, что именно будет сделано (чек‑лист), без создания файлов фазы. После выполнения фазы в этот раздел добавляется отметка статуса (done) и ссылка на файл `phases/phase_*_summary.md`.
  - Рекомендуемый формат для каждой фазы внутри «Список фаз»:
    - Заголовок `### ⏳/✅ Фаза N: Короткий заголовок`
    - Краткий перечень задач (чек‑лист, маркеры `-`)
...
### 6.3 Пример раздела «Список фаз» в AGENT_TASK_SUMMARY.md (ПЛАН до выполнения)
