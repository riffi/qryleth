...
## 10. Процедурные облака и глобальный ветер

- Следуем тем же принципам, что и для террейна/биомов: спецификация → генератор → слой сцены → публичные методы SceneAPI.
- Внешний вид облаков задаётся метапараметрами `appearance` (preset/size/softness/dynamics/color/variance); конкретные параметры рендера вычисляет генератор детерминированно по `seed`.
- Слой `GfxLayerType.Clouds` хранит `GfxCloudsConfig` (минимально — список `items`). Визуальные поля конкретного облака допускают `advancedOverrides`.
- Глобальный ветер (`environment.wind: { direction: [x,z], speed }`) является частью состояния сцены; направление всегда нормализуется. Визуальные компоненты (облака) используют его для дрейфа.
- SceneAPI предоставляет методы управления ветром и генерацией облаков; все методы снабжаем подробными комментариями на русском.
