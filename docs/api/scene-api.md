...
> Примечание (процедурные объекты)
>
> Если объект в библиотеке/сцене является процедурным деревом (`objectType: 'tree'`), его `primitives` могут быть пустыми.
> SceneAPI и слой рендера автоматически восстанавливают примитивы для расчёта `boundingBox` и отображения на основе `treeData` (детерминированная генерация по `seed`).
