...
Редактор сцен использует доменные типы из раздела API. Подробности и определения см. в:
...
Состояние редактора сцен инкапсулировано в `sceneStore`. Использование, состав состояния и рекомендации по селекторам описаны в:
- [State Management Overview](../../api/stores/README.md)
