 ...
 * Содержит набор правил размещения, которые в совокупности дают точки для этой страты.
 */
export interface GfxBiomeStratum {
  /** Имя страты (для UI/логов) */
  name: string
  /** Набор правил размещения для страты */
  rules: GfxBiomePlacementRule[]
}

/**
 * Правило размещения: базовая единица конфигурации внутри страты.
 * На первой итерации структура пустая (используются глобальные параметры биома).
 * В следующих итерациях сюда добавятся локальные параметры и выбор источников.
 */
export interface GfxBiomePlacementRule {
  /** Опциональное имя правила (для UI/логов) */
  name?: string
  /**
   * Локальная плотность для данного правила (на 100×100).
   * Если не задано — на этапе скелета может браться доля от глобальной плотности биома.
   */
  densityPer100x100?: number
  /**
   * Локальный алгоритм распределения точек для правила.
   * Если не задан — используется глобальный `scattering.distribution` биома.
   * Варианты:
   * - 'random' — равномерная случайная выборка с учетом fade/bias;
   * - 'poisson' — Poisson‑disk (blue noise) с минимальным расстоянием между точками.
   */
  distribution?: 'random' | 'poisson'
  /**
   * Локальное минимальное расстояние между точками (актуально для 'poisson').
   * Если не задано — используется глобальный `scattering.minDistance` биома.
   * Для 'random' может игнорироваться.
   */
  minDistance?: number
  /**
   * Локальные настройки поведения на краях области (fade/bias).
   * При отсутствии используются глобальные параметры биома.
   */
  edge?: GfxBiomeEdgeFalloff
  /**
   * Локальные параметры случайных трансформаций (yaw/scale/offsetXZ) для инстансов данного правила.
   * При отсутствии используются глобальные параметры биома.
   */
  transform?: GfxBiomePlacementTransform
  /**
   * Локальный выбор источников из библиотеки для данного правила.
   * Если не задан — используется глобальный фильтр биома (scattering.sources).
   */
  sourceSelection?: GfxBiomeSourceFilter
}
