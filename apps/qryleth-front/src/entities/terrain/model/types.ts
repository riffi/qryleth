...
export interface GfxLandscape {
  ...
  name?: string
  /**
   * Признак видимости площадки в сцене.
   * Если не задано, по умолчанию считается видимой (true).
   * Хранится в сторе сцены и влияет на рендер.
   */
  visible?: boolean
  ...
  material?: {
    ...
    /**
     * Много-текстурная раскраска по высоте (аналогично multiColor, но с текстурами вместо цветов).
     * Слои сортируются по высоте; между соседними высотами выполняется плавная интерполяция.
     * Поддержка до 4 слоёв на одну площадку.
     */
    multiTexture?: {
      /** Список слоёв текстур с опорной высотой (мировой Y) */
      layers: Array<{
        /** Идентификатор текстуры из реестра ландшафта */
        textureId: string
        /** Опорная высота Y (мировая), на которой слой является «центральным» для смешивания */
        height: number
