...
export const useObjectContextPrompt = () => {
  ...
  const systemPrompt = useMemo(() => {
    const basePrompt = `
Ты помощник для редактирования 3D объектов в ObjectEditor. Объекты создаются в системе координат Three.js. Ось Y смотрит вверх.
PivotPoint - в геометрическом центре примитива
