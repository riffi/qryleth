...
export class MultiColorProcessor {
  ...
  /**
   * ВЕРШИННАЯ окраска — возвращает Float32Array (r,g,b per vertex).
  ...
  generateVertexColors(
    ...
    for (let i = 0; i < count; i++) {
      ...
      this.samplePaletteByHeight(y, c)
  ...
  generateFaceColoredGeometry(
    ...
    const pushVertex = (vx: number, vy: number, vz: number, color: THREE.Color, nx?: number, ny?: number, nz?: number) => {
    ...
    for (let f = 0; f < triCount; f++) {
      ...
      // возьмём цвет по центру треугольника (стабильнее, чем первая вершина)
      center.set((vA.x + vB.x + vC.x) / 3, (vA.y + vB.y + vC.y) / 3, (vA.z + vB.z + vC.z) / 3)
      this.samplePaletteByHeight(center.y, faceColor)
      ...
      // продублируем вершины, у всех цвет одинаковый
      pushVertex(vA.x, vA.y, vA.z, faceColor)
      pushVertex(vB.x, vB.y, vB.z, faceColor)
      pushVertex(vC.x, vC.y, vC.z, faceColor)
    }
  ...
  private samplePaletteByHeight(y: number, out: THREE.Color) {
    // поиск двух ближайших стопов по высоте и линейная интерполяция цветов
    const stops = this.palette
    if (stops.length === 0) { out.set('#ffffff'); return }

    if (y <= stops[0].height) { out.set(stops[0].color); return }
    if (y >= stops[stops.length - 1].height) { out.set(stops[stops.length - 1].color); return }
