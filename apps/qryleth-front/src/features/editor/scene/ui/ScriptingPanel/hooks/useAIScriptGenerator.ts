...
export function useAIScriptGenerator() {
  ...
  const buildSystemPrompt = useCallback((): string => {
    return [
      ...
      '',
      '14) createProceduralLandscape(spec: GfxProceduralTerrainSpec, opts?: { name?: string; center?: [number, number]; size?: { width: number; depth: number }; material?: { color?: string; multiColor?: GfxMultiColorConfig } }): Promise<{ success: boolean, layerId?: string, error?: string }>',
      ...
      '  seed: 2468',
      '};',
      'const res = await sceneApi.createProceduralLandscape(spec, { name: "Холмистый ландшафт" });',
      'console.log(res);',
      '```',
      '',
      ...
      '  seed: 7777',
      '};',
      'const duneLayer = await sceneApi.createProceduralLandscape(dunes, { name: "Песчаные дюны" });',
