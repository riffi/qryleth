...
export interface PrimitiveGroup {
  ...
  // Для удобства навигации (опционально, вычисляется динамически)
  children?: string[]; // UUID дочерних групп
}
...
export interface GfxObject {
  ...
  // Привязка примитивов к группам по UUID, а не индексам
...
- **Примитивы**: `generatePrimitiveUuid()` в `src/shared/lib/uuid.ts`
- **Группы**: `generateGroupUuid()` в том же файле  
- **Префиксы**: "prim_", "group_" для различения типов (опционально)
...
### Unit тесты:
- Утилиты работы с группами (buildGroupTree, findGroupChildren)
- Разрешение конфликтов при импорте
- Генерация UUID
- Селекторы zustand store

### Integration тесты:
- Создание и удаление групп через UI  
- Drag-and-drop примитивов между группами
- Импорт объектов с группами
- Рендеринг иерархических групп

### E2E тесты:
- Полный цикл работы с группами в ObjectEditor
- Импорт сложного объекта с множественной иерархией
...
- **Создать PrimitiveGroup**: Интерфейс с полями uuid, name, visible, parentGroupUuid, isImported, sourceObjectUuid, children
...
- **Actions для иерархии**: moveGroupToParent, getGroupHierarchy
...
- **Drag-and-drop**: Перемещение примитивов и групп между уровнями
...
✅ Индикатор "импорт" отображается для isImported групп
...
**Описание**: Реализовать функциональность импорта объектов из библиотеки с полным сохранением иерархической структуры групп.

**Детальные действия**:
- **Функция импорта**: `importObjectAsGroup(sourceObject, targetObject, groupName)`
- **Разрешение конфликтов**: Генерация новых UUID, суффиксы для имен
- **Сохранение иерархии**: Рекурсивное копирование структуры групп
- **UI импорта**: Модальное окно выбора объекта с превью структуры групп
- **Предварительный просмотр**: Показ иерархии групп импортируемого объекта

**Критерии успешности**:
✅ `importObjectAsGroup()` корректно копирует все примитивы с новыми UUID
✅ Иерархия групп полностью сохраняется с правильными parentGroupUuid  
✅ Конфликты имен разрешаются через суффиксы "(импорт N)"
✅ UI показывает превью структуры групп перед импортом
✅ Импортированные группы помечены isImported: true и sourceObjectUuid
✅ Обратная совместимость: импорт объектов без групп создает одну группу

**Файлы для изменения**:
- `src/features/object-editor/lib/objectEditorApi.ts`
- `src/features/object-editor/ui/ObjectManagementPanel/ImportObjectModal.tsx` (новый)
- `src/features/object-editor/ui/ObjectManagementPanel/GroupStructurePreview.tsx` (новый)
