...
- add_primitives: добавить один или несколько примитивов (массовая операция)
- modify_primitive: изменить существующий примитив (позиция, поворот, масштаб)
- remove_primitive: удалить примитив по индексу
- duplicate_primitive: дублировать примитив с смещением

🎨 **Управление материалами:**
- create_material: создать новый материал с настройками цвета, металличности, шероховатости
- update_material: обновить существующий материал
- assign_material: назначить материал группе примитивов
- remove_material: удалить материал (с опциональной заменой)
- duplicate_material: дублировать материал с изменениями

📊 **Анализ и оптимизация:**
- analyze_object: анализировать структуру объекта
- optimize_object: оптимизировать объект для производительности
- validate_object: проверить объект на корректность
- suggest_improvements: предложить улучшения
- calculate_stats: подсчитать статистику объекта
- generate_variations: сгенерировать вариации объекта
...
Предлагай изменения с учетом существующей структуры объекта.`
  ...
  const contextualHints = useMemo(() => {
    ...
    if (objectInfo.isEmpty) {
      hints.push('Создай базовую структуру с помощью add_primitives')
    ...
    if (objectInfo.primitivesCount > 5) {
      hints.push('Можно проанализировать структуру с помощью analyze_object')
      hints.push('Рассмотри оптимизацию для лучшей производительности')
    }

    if (objectInfo.selectedPrimitiveIds.length > 0) {
      hints.push('Можно изменить выбранные примитивы с помощью modify_primitive')
